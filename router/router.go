...
func SifRouter(r *gin.Engine) {
	...
	// session
	cookieSecret := config.Conf.Settings.CookieSecret
	if cookieSecret == "" {
		cookieSecret = "llsif"
	}
	store := cookie.NewStore([]byte(cookieSecret))
	r.Use(sessions.Sessions("llsif", store))
	r.Use(middleware.AntiScanner())
	r.Use(middleware.RateLimitRPM(config.Conf.Settings.RateLimitRPM))
	r.Use(middleware.RecoverSif)
	...
	// WebUI
	w := r.Group("admin").Use(middleware.AdminGate(config.Conf.Settings.AdminEnabled, config.Conf.Settings.AdminAllowIPs), middleware.WebAuth)
	{
